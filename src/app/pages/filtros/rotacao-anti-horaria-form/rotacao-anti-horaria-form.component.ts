...
@Component({
  selector: 'rotacao-anti-horaria-form',
  templateUrl: './rotacao-anti-horaria-form.component.html',
  styleUrls: ['./rotacao-anti-horaria-form.component.scss'],
  animations: [ slideInOut ]
})
export class RotacaoAntiHorariaFormComponent implements OnInit {
  ...
  voltarMenu(){
    this.onVoltar.emit('girarAH');
  }

  aplicar(){
    this.carregando = true;
    this.onLoading.emit(this.carregando);
    let nomeFiltro = 'girarAH';
    ...
    this.filtrosService.requisitarFiltro(nomeFiltro,parametrosFiltro,this.imagem).subscribe(response => {
      ...
      this.onFiltroAplicado.emit({filtro : 'girarAH',imagemProcessada : response, msg : "Imagem rotacionada!"});
    }, err => {
      this.carregando = false;
      console.log(err);
    });
