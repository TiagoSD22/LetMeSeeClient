...
@Component({
  selector: 'prewitt-form',
  templateUrl: './prewitt-form.component.html',
  styleUrls: ['./prewitt-form.component.scss'],
  animations: [ slideInOut ]
})
export class PrewittFormComponent implements OnInit {
  ...
  voltarMenu(){
    this.onVoltar.emit('prewitt');
  }

  aplicar(){
    this.carregando = true;
    this.onLoading.emit(this.carregando);
    let nomeFiltro = 'prewitt';
    ...
    this.filtrosService.requisitarFiltro(nomeFiltro,parametrosFiltro,this.imagem).subscribe(response => {
      ...
      this.onFiltroAplicado.emit({filtro : 'prewitt',imagemProcessada : response, msg : "Bordas detectadas por Prewitt!"});
    }, err => {
      this.carregando = false;
      console.log(err);
    });
